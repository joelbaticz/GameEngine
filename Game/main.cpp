...
int main(int, char**) {
	...
	{
		//GameEnigne::GetStatus() != STATUS_OK
		//GameEngine::GetLastError ==> string
		GameEngine::UnInit();
	...
	//Node* child = startScene->GetFirstChild();
	//int count = 0;
	//while (child != nullptr)
	//{
	//	LogEngine::Log(LOG_TRACE, "Main", "StartScene Child[" + std::to_string(count) + "] = " + child->GetName());
	//
	//	//if (typeid(child) == CanvasNode*)
	//	if (dynamic_cast<CanvasNode*>(child)) ((CanvasNode*)child)->Draw();
	//	
	//	child = startScene->GetNextChild(child);
	//	count++;
	//}
	...
	std::cout << "[TRACE] - Main: Finished without errors. Exiting." << std::endl;
