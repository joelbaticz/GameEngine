...
void Checkbox::SetSubComponentSizes()
{
	...
	textSize->SetWidth(this->GetSize()->GetWidth() - 22);
...
void Checkbox::Init(Node* parent, std::string name, std::string caption, int x, int y, int width, int height)
{
	...
	Texture* checkboxTexture = AssetManager::CreateTexture("Checkbox.png");
	checkboxTexture->SetHorizontalCount(1);
	checkboxTexture->SetVerticalCount(8); //Possible states: Normal, Hover, Pressed, KeyboardFocus, Disabled
	m_CheckboxSprite = new SpriteNode();
	...
	m_CheckboxSprite->SetLocation(checkboxLocation);
	((SpriteNode*)m_CheckboxSprite)->SetTexture(checkboxTexture);
	((SpriteNode*)m_CheckboxSprite)->SetTextureIndex(0);
	((SpriteNode*)m_CheckboxSprite)->SetCornerSize(1);
	...
	m_CaptionTextNode->SetLocation(textLocation);
...
Checkbox::Checkbox()
{
	LogEngine::Log(LOG_TRACE, "Checkbox", "Constructor called.");

	Init(nullptr, "Checkbox", "Caption", DEFAULT_X, DEFAULT_Y, DEFAULT_WIDTH, DEFAULT_HEIGHT);
}

Checkbox::Checkbox(Node* parent, std::string name, std::string caption, int x, int y, int width, int height)
{
	LogEngine::Log(LOG_TRACE, "Checkbox", "Constructor called.");
...
Checkbox::~Checkbox()
{
	LogEngine::Log(LOG_TRACE, "Checkbox", "Destructor called.");
